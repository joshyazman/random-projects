...
incidence_county_day_init <- read_csv('/Users/joshyazman/Desktop/covid-political-analysis/county-level-tabs/combined_nyt_cnty.csv')%>%
  mutate(twoway_16_label = apply_labels(twoway_16_cat),
         twoway_08_label = apply_labels(twoway_08_cat),
         twoway_12_label = apply_labels(twoway_12_cat),
...
cnty <- read_delim('/Users/joshyazman/Desktop/covid-political-analysis/county-level-tabs/countypres_2000-2016.tab', delim = '\t')%>%
  ...
  mutate(dem_twoway = candidatevotes/twoway_votes,
         ...
         county_fips = stringr::str_pad(FIPS,5,side = 'left', pad = '0'))%>%
  group_by(year, state_name, state_code = state_po, county_fips)%>%
...
all_states <- c('US',unique(incidence_county_day_init$state_name))
all_regions <- c('US',unique(incidence_county_day_init$region))
...
plot_outcomes <- function(state_code_filter = all_states, 
  ...
  if(state_code_filter == 'US' & region_filter == 'US') {
    incidence_county_day_init_filtered <- incidence_county_day_init
    subtitle_text <- element_blank()
  } else if (state_code_filter != 'US' & region_filter == 'US') {
    incidence_county_day_init_filtered <- incidence_county_day_init%>%
      filter(state_name %in% state_code_filter)
  ...
  weekly_by_hrc <- incidence_county_day_init_filtered %>%
    ...
    select(date, incidence_week, metric, state_county_fips, 
           result = twoway_16_label, order_var = twoway_16_cat, 
           new_cases, total_deaths, n_rvs)%>%
    group_by(metric, incidence_week, result, order_var)%>%
    summarise(total_deaths = sum(total_deaths), ## Don't actualy use this until you figure out how to properly roll it up
              new_cases = sum(new_cases),
              new_cases_per_voter = sum(new_cases)/sum(n_rvs))%>%
    group_by(metric, incidence_week)%>%
    mutate(pct_new_cases = new_cases/sum(new_cases),
           pct_new_cases_per_voter = new_cases_per_voter/sum(new_cases_per_voter),
           pct_total_deaths = total_deaths/sum(total_deaths))%>%
    ungroup()
  
  weekly_by_ots <- incidence_county_day_init_filtered %>%
    mutate(metric = 'Cases by Obama - Trump Swing')%>%
    select(date, incidence_week, metric, 
           state_county_fips, result = obama_trump_label, 
           order_var = obama_trump_cat, new_cases, 
           total_deaths, n_rvs)%>%
    group_by(metric, incidence_week, result, order_var)%>%
    summarise(total_deaths = sum(total_deaths), ## Don't actualy use this until you figure out how to properly roll it up
              new_cases = sum(new_cases),
  ...
  pop_by_hrc <- incidence_county_day_init_filtered %>%
    ...
    select(date, incidence_week, metric, state_county_fips, 
           result = twoway_16_label, order_var = twoway_16_cat, 
           new_cases, total_deaths, n_rvs, pop_total)%>%
    group_by(metric, incidence_week, result, order_var)%>%
    summarise(total_deaths = sum(total_deaths), ## Don't actualy use this until you figure out how to properly roll it up
              new_cases = max(pop_total),
              new_cases_per_voter = sum(new_cases)/sum(pop_total))%>%
    group_by(metric, incidence_week)%>%
    mutate(pct_new_cases = new_cases/sum(new_cases),
           pct_new_cases_per_voter = new_cases_per_voter/sum(new_cases_per_voter),
           pct_total_deaths = total_deaths/sum(total_deaths))%>%
...
# Make a static plot of cases by day and category
```{r, fig.width=12}
tot 

tot_anim <- animate(tot, width = 800, height = 350, end_pause = 2, start_pause = 2)
anim_save(filename = '/Users/joshyazman/Desktop/covid-political-analysis/Share of Weekly New Cases by Election Result.gif')

pop_adj <- 

pop_adj_anim <- animate(pop_adj, width = 800, height = 350, end_pause = 2, start_pause = 2)
anim_save(filename = '/Users/joshyazman/Desktop/covid-political-analysis/Share of Weekly New Cases per Voter by Election Result.gif')
